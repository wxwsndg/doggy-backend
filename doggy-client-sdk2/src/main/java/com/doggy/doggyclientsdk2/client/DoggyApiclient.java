package com.doggy.doggyclientsdk2.client;
...
import com.doggy.doggyclientsdk2.model.User;

import java.util.HashMap;
import java.util.Map;

import static com.doggy.doggyclientsdk2.utils.SignUtils.getSign;
...
public class DoggyApiclient {
    ...
    //使用POST方法向服务器发送测试对象，并获取服务器返回的结果
    public String getPerDayALineByPost(){
        String gs = "ceshi";//无参调用每日一句
        String json = JSONUtil.toJsonStr(gs);
        //使用HttpRequest工具发起POST请求，并获取服务器的响应
        HttpResponse httpResponse = HttpRequest.post(GATEWAY_HOST + "/api/perdayaline/post")
                .addHeaders(getHeadMap(json)) //添加前面构造的请求头
                .body(json)  //将JSON字符串设置为请求体
                .execute();//执行请求
        //打印服务器返回的状态码
